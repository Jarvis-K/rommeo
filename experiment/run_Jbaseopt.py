...
from maci.learners import MAVBAC, MASQL, ROMMEO, JSQL, GPF
from maci.misc.sampler import MASampler, JSampler
...
from maci.get_agents import ddpg_agent, masql_agent, pr2ac_agent, rom_agent, jsql_agent, gpf_agent
...
def objective(trail, arglist):
    ...
    sampler = JSampler(agent_num=agent_num, joint=True, global_reward=arglist.global_reward, max_path_length=25, min_pool_size=100, batch_size=batch_size, do_nego = arglist.do_nego)
    ...
    with U.single_threaded_session():
        for i, model_name in enumerate(model_names):
            if 'PR2AC' in model_name:
                ...
                agent = jsql_agent(model_name, i, env, M, u_range, base_kwargs,  lr=lr, n_pars=n_pars, game_name=game_name)
            elif model_name == 'GPF':
                agent = gpf_agent(model_name, i, env, M, u_range, base_kwargs,  lr=lr, n_pars=n_pars, batch_size = batch_size, game_name=game_name)
...
if __name__ == '__main__':
    ...
    study_name = 'N3D-' + str(arglist.max_steps) + '-' + arglist.model_names_setting + str(arglist.do_nego)+ '(' + str(arglist.x2) + ',' + str(arglist.y2) + ',' + str(arglist.s2) + ')'

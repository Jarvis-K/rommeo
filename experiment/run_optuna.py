    ...
    with U.single_threaded_session():
        for i, model_name in enumerate(model_names):
            if 'PR2AC' in model_name:
                ...
                agent = rom_agent(model_name, i, env, M, u_range, base_kwargs, game_name=game_name, lr=lr, n_pars=n_pars)
            else:
                if model_name == 'DDPG':
                    joint = False
...
if __name__ == '__main__':
    ...
    study_name = 'NN3D-' + str(arglist.max_steps) + '-' + arglist.model_names_setting + '(' + str(arglist.x2) + ',' + str(arglist.y2) + ',' + str(arglist.s2) + ')'
